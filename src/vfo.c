  ...
  if (vfl->eq_x != 0) {
    ...
    if (transmitter->eq_enable) {
      ...
      if (transmitter->eq_gain[0] == 0.0) {
        ...
        snprintf(temp_text, 32, "TxEQ %+d", (int) transmitter->eq_gain[0]);
      }
    } else {
      cairo_set_source_rgba(cr, COLOUR_SHADE);
      snprintf(temp_text, 32, "TxEQ");
